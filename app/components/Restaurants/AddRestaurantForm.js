  ...
  const addRestaurant = () => {
    if (!restaurantName || !restaurantAddress || !restaurantDescription) {
      toastRef.current.show(
        "Todos los campos del formulario son obligatiorios"
      );
    } else if (size(imagesSelected) === 0) {
      toastRef.current.show("El restautante tiene que tener al menos una foto");
  ...
  useEffect(() => {
    (async () => {
      ...
      if (statusPermissions !== "granted") {
        toastRef.current.show(
          "Tienes que aceptar los permisos de localizacion para crear un restarurante",
          3000
        );
      } else {
